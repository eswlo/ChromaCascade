...
template <template <class T> class OrderingStructure> animation filler::Fill(FillerConfig& config)
{
	 ...
	 * add the point to the ordering structure (os), or when we take it off. In our test cases,
	...
	vector<PixelPoint> visitedPixs;
	PixelPoint seedpoint = config.seedpoint;
	os.Add(seedpoint);
	visitedPixs.push_back(seedpoint);
	unsigned int width = config.img.width();
	unsigned int height = config.img.height();
	// PNG* renderedImg = new PNG(width, height);
	// renderedImg.resize(width, height);

	while (!os.IsEmpty()) {
		//get the pp from os
		PixelPoint removed = os.Remove(); 
		//get the pixel from the img to be processed
		RGBAPixel* targetPixtoFill = config.img.getPixel(removed.x, removed.y);
		//pick color/pattern
		ColorPicker* pick = config.picker;
		RGBAPixel colorFromPicker = ((*pick)(removed));
		//pass the selected color to the pixel in the processed img
		*targetPixtoFill = colorFromPicker;
		framecount++;

		if (framecount >= config.frameFreq && (framecount % config.frameFreq == 0)) {
			anim.addFrame(config.img);
		}
		
		//add northern neighbour
		if (removed.y >= 1) {
			unsigned int x = removed.x;
			unsigned int y = removed.y - 1;
			addToVisitedPixs(x, y, config, visitedPixs, os);
		}

		//add eastern neighbour
		if (removed.x <= width - 2) {
			unsigned int x = removed.x + 1;
			unsigned int y = removed.y;
			addToVisitedPixs(x, y, config, visitedPixs, os);
		}

		//add southern neighbour
		if (removed.y <= height - 2) {
			unsigned int x = removed.x;
			unsigned int y = removed.y + 1;
			addToVisitedPixs(x, y, config, visitedPixs, os);
		}
...
void filler::addToVisitedPixs(unsigned int x, unsigned int y, FillerConfig& config, vector<PixelPoint>& visitedPixs, OrderingStructure<PixelPoint>& os)
{
	//check if the pix is already processed
	bool isAdded = false;
	for (size_t i = 0; i < visitedPixs.size(); i++) {
		if ((visitedPixs[i].x == x) && (visitedPixs[i].y == y)) {
			isAdded = true;
			break;
		}
	}
	if(!isAdded) {
		...
		if (config.seedpoint.color.distanceTo(*pixel) <= config.tolerance) {
			PixelPoint newpp(x, y, *pixel);
			os.Add(newpp);
			visitedPixs.push_back(newpp);
		}
	}
